...
import { useAddRegistrarGeneralMutation, useDeleteRegistrarGeneralMutation, useInactiveOrActiveRegistrarGeneralMutation, useUploadRegistrarGeneralSignatureMutation } from "@/states/api/settingsApiSlice";
...
const schema: z.ZodType<AddRegistrarDto> = z.object({
    ...
    signature: z.instanceof(File, { message: "Signature is required and must be a file" }),
});
...
export default function useAddRegistrar(props: Props) {
    ...
    const { control, handleSubmit, register, setValue,formState: { errors }, reset } = useForm<AddRegistrarDto>({
    ...
    const [
        addRegistrarGeneral, 
        {
            isLoading: creatingRegistrarGeneral, 
    ...
    const [inactivateOrActivate, {
        isLoading: inactivatingOrActivating,
    ...
    const onSubmit = async (data: AddRegistrarDto) => {
        try {
            ...
            const response = await addRegistrarGeneral({...rest, parameterType: "REGISTRAR_GENERAL"}).unwrap();
            if (response) {
                if (signature) {
                    const formData = new FormData();
                    formData.append("file", signature);
                    const uploadResponse = await uploadSignature({ id: response?.data.id, formData }).unwrap();
                    if (uploadResponse) {
            ...
            toast.error(error?.data?.message || "An error occurred while adding registrar general");
        }
    }

    const handleInactivateOrActivate = async (id: string, action: 'activate' | 'inactivate') => {
        try {
            ...
            toast.error(error?.data?.message || `An error occurred while ${action}ing registrar general`);
        }
    }
    ...
    const handleDeleteRegistrarGeneral = async (id: string) => {
        try {
            ...
            toast.error(error?.data?.message || "An error occurred while deleting registrar general");
        }
    }
